...
namespace WordsMatching
{
	...
	public class SentenceSimilarity
	{
        ...
        public float GetScore1(string string1, string string2)
        {
            Tokeniser tok = new Tokeniser();
            tok.UseStemming = false;
            ...
            _source = tok.Partition(string1);
            _target = tok.Partition(string2);
            int _targetLength = _target.Length;
            if (_source.Length == 0 || _targetLength == 0)
                return 0F;

            if (_targetLength > 1)
            {
                string [] newTarget = new string[1];
                newTarget[0] = _target[0];
                for (int i = 1; i < _targetLength; i++)
                    newTarget[0] = newTarget[0] + " " + _target[i];
                _target = newTarget;

                int l = _source.Length - _targetLength + 1;
                if (l <= 0)
                    l = 1;
                string[] newSource = new string[l];
                for (int j = 0; j < l; j++)
                {
                    newSource[j] = _source[j];
                    for (int j1 = 1; j1 < _targetLength; j1++)
                    {
                        newSource[j] = newSource[j]  + " " + _source[j + j1];
                    }
                }
                _source = newSource;
            }
